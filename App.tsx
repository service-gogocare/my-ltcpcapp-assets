  ...
  const handleSendResult = useCallback(async () => {
    ...
    try {
        ...
        if (resJson.status === 'error') {
            throw new Error(resJson.message || '後端處理失敗');
        }
    } catch (e) {
        // 如果不是 JSON，但回傳 ok，通常代表已寄出 (no-cors mode)
    }
  }, [points, results, userId]);
          ...
          <header className="text-center relative mb-8">
            {(viewMode === 'detailed' || viewMode === 'manualInput') && (
                <button 
                    ...
                    className="absolute left-0 bottom-0 sm:top-1/2 sm:bottom-auto sm:-translate-y-1/2 flex items-center px-2 py-1 text-gray-600 dark:text-gray-400 hover:text-brand-primary dark:hover:text-white transition-colors rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-accent"
                    aria-label="返回首頁"
                >
              ...
              <div className="relative z-10 text-center">
                  ...
                  <div className="mt-2">
                      <button 
                          ...
                          className="text-sm text-gray-500 dark:text-gray-400 hover:text-brand-primary dark:hover:text-brand-accent underline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-accent rounded"
                      >
                          《使用條款》
                      </button>
                  </div>
